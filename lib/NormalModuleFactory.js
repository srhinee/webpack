...
class NormalModuleFactory extends Tapable {
	constructor(context, resolverFactory, options) {
		...
		this.hooks.factory.tap("NormalModuleFactory", () => (result, callback) => {
			//mark normalModuleFactory.hook.resolver
			...
			resolver(result, (err, data) => {
				...
				//mark normalModuleFactory.hook.afterResolve 空hook 啥也没做 data=>result
				this.hooks.afterResolve.callAsync(data, (err, result) => {
					//main 5  factory createModule
					if (err) return callback(err);
					...
					if (!createdModule) {
						...
						//result<= resolver process callback return
			...
			asyncLib.parallel(
				[
					...
					callback => {
						...
						//resolver 3 resolve normalPath and inlineLoaderPath
				...
				(err, results) => {
					...
					//resolver 4 resolve config loader and sorted loader
					asyncLib.parallel(
						[
						...
						(err, results) => {
							if (err) return callback(err);
							// loader 是上一步内联解析出的,这一步给loader排序来决定执行顺序
							...
							process.nextTick(() => {
								...
								//resolver 5 resolver callback and init module options
								callback(null, {
									context: context,
									request: loaders
										.map(loaderToIdent)
	...
	create(data, callback) {
		...
		this.hooks.beforeResolve.callAsync(
			...
			(err, result) => {
				...
				// Ignored
				if (!result) return callback();
				//main 4 resolver process
				//mark normalModuleFactory.hook.factory
