 ...
 * @property {(data: ModuleFactoryCreateData, callback: ModuleCallback) => any}
 *   create
 */
 ...
 * @property {boolean=} immutable true, if the asset can be long term cached
 *   forever (contains a hash)
 * @property {number=} size size in bytes, only set after asset has been
 *   emitted
 * @property {boolean=} development true, when asset is only used for
 *   development and doesn't count towards user-facing assets
 * @property {boolean=} hotModuleReplacement true, when asset ships data for
 *   updating an existing application (HMR)
 */
...
/**
 * @param {DependenciesBlockVariable[]} variables DepBlock Variables to iterate
 *   over
 * @param {DepBlockVarDependenciesCallback} fn callback to apply on iterated
 *   elements
 * @returns {void}
 */
...
class Compilation extends Tapable {
	...
	/**
	 * @param {string | (function(): string)} name name of the logger, or
	 *   function called once to get the logger name
	 ...
	 * @returns {AddModuleResult} returns meta about whether or not the module
	 *   had built had an issuer, or any dependnecies
	 */
	 ...
	 * @returns {Module|undefined} attempt to search for module and return it,
	 *   else undefined
	 */
	 ...
	 * @param {Dependency[]=} dependencies optional dependencies from the module
	 *   to be built
	...
	processModuleDependencies(module, callback) {
		...
		try {
			//tip 构建dep 数组[{factory,dependencies}]
	 ...
	 * @param {SortedDependency[]} dependencies set of sorted dependencies to
	 *   iterate through
	 ...
	 * @param {function} callback callback for when dependencies are finished
	 *   being added
	 * @returns {void}
	 */
	 ...
	 * @param {ModuleChainCallback} callback callback for when module chain is
	 *   complete
	 * @returns {void} will throw if dependency instance is not a valid
	 *   Dependency
	 */
				...
				(err, module) => {
					...
					const afterBuild = () => {
						if (addModuleResult.dependencies) {
							//main 11 processModuleDependencies
	...
	seal(callback) {
		...
		while (
			...
			//mark compilation.hook.optimizeDependencies
	...
	/**
	 * @param {Module[]} modules the modules array on compilation to perform the
	 *   sort for
	 * @returns {void}
	 */
	sortModules(modules) {
	 ...
	 * @param {DependencyLocation} loc the location from with the chunk group is
	 *   referenced (inside of module)
	 * @param {string} request the request from which the the chunk group is
	 *   referenced
	 ...
	 * and first looks to see if any named chunks already exist and reuse that
	 * chunk instead.
	 *
	...
	sortItemsWithChunkIds() {
		 ...
		 * updated whenever other references (having a chunk id) are sorted. This
		 * preserves the hash integrity
		 *
		 * @param {WebpackError} a first WebpackError instance (including
		 *   subclasses)
		 * @param {WebpackError} b second WebpackError instance (including
		 *   subclasses)
	 ...
	 * @param {string} filename used to get asset path with hash
	 * @param {TODO=} data // TODO: figure out this param type
	 * @returns {{ path: string, info: AssetInfo }} interpolated path and asset
	 *   info
	 */
	...
	/**
	 * This function allows you to run another instance of webpack inside of
	 * webpack however as a child with different settings and configurations (if
	 * desired) applied. It copies all hooks, plugins from parent (or top level
	 * compiler) and creates a child Compilation
	 *
	 * @param {string} name name of the child compiler
	 * @param {TODO} outputOptions // Need to convert config schema to types for
	 *   this
